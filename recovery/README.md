# sharenet-recovery — R7-002 (durable recovery attempts) + R7-003 (alternate route/gateway recovery) + R7-004 (replacement circuit) + R7-005 (retry/backoff) + R7-006 (concurrent recovery/second failure/cleanup)
...
| `recoveries` + `cleanup_terminal` (R7-006) | `src/driver.rs` | The cross-circuit coordination view (every circuit with history + its typed state — in flight / retryable / awaiting retry / terminal — in the deterministic id order, the daemon scheduler's pure input) and the terminal cleanup: prune a terminal recovery's ABANDONED history (retaining the terminal record + high-water + §11 anchor + zeroization fact), age-gated and typed — the revocation LEDGER is never touched (L015), and a cleaned circuit stays `recovery_already_complete` (cleanup can never resurrect a completed recovery). |
...
- **concurrency + multiprocess, R7-006** — the coordination view is
  consistent while recoveries mutate underneath (racing readers +
  writers: no circuit disappears, no torn terminal states), racing
  cleanups are safe (exactly-once pruning, the terminal fact survives
  every racer); across processes: the full lifecycle (two abandoned
  rounds → terminal third → replacement), the typed `cleanup_too_early`
  refusal, the old-enough cleanup pruning to exactly the terminal
  record (which still blocks new attempts), the SECOND FAILURE (the
  replacement revoked in a later process opens its own fresh
  recovery), and the two-circuit coordination view from one more
  process, deterministically ordered.
- **multiprocess (`tests/multiprocess.rs`, 4)** — through the REAL
...
```sh
cargo test          # 61 unit + 7 concurrency + 5 restart + 13 adversarial + 4 multiprocess — all green
```

## Deliberately NOT done here (honest scope)

The R7-006 coordination VIEW is the daemon scheduler's pure input —
which recovery gets the next contact slot is the daemon's arbitration
policy composed on top (the R7-005 gate likewise stays a pure query;
the daemon's timers and wakeups are its own wiring, and the policy
never sleeps). Cleanup bounds the ATTEMPT LOG only — the L015 ledger
never forgets. The
