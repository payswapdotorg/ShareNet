//! `recovery_probe` — TEST SCAFFOLDING for the R7-003 "multiprocess"
//! verify level: a REAL separate process per §11 pipeline stage, each
//! seeing the recovery state ONLY through the bytes on disk (the two
//! durable files in the driver's directory). The ShareNet daemon is
//! modeled as a sequence of probe invocations:
//!
//! - process 1 (`setup`, `open-attempt`) admits a revocation for a
...
//! - process 3 (`state`) reloads from disk and reports the terminal
//!   state.
//!
...
//! re-derivation of the selection).
...
use sharenet_recovery::{
    GatewayCandidate, RecoveryDriver, RecoveryError, RecoveryStep, SelectedGateway,
};
